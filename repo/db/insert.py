...
from lib import connect_db, fetch, enforce_logging, list_nested_paths


@enforce_logging(main=False)
def log_error(expr, error, args):
    if expr == False:
...
def path_join(base_dir, suffixes, isdir):
...
def get_schema():
    schema_dict = dict()
    with open("schema.json", 'r') as schema_json:
